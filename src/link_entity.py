...
class CandGen:
    ...
    def get_l2s_map(self, eids, eids_google, eids_hindi, eids_google_maps, eids_spell, eids_gtrans, eids_trans,
    ...
    def compute_hits_for_ta(self, docta, outfile, only_nils=False, args=None):
        ...
        for idx, cons in enumerate(candgen_view_json["viewData"][0]["constituents"]):
            ...
            eids, eids_google, eids_hindi, eids_google_maps, eids_spell, eids_gtrans, eids_trans = [], [], [], [], [], [], []

            mention_cheap_dict = {} #mention2eid[self.lang]
            if query_str in mention_cheap_dict:
                eids_total = mention_cheap_dict[query_str]
            else:
                eids, eids_google, eids_hindi, eids_google_maps, eids_spell, eids_gtrans, eids_trans = self.get_lorelei_candidates(query_str, ner_type=ner_type, args=args)
                eids_total = eids + eids_google + eids_hindi + eids_google_maps + eids_spell + eids_gtrans + eids_trans
            ...
            l2s_map = self.get_l2s_map(eids, eids_google, eids_hindi, eids_google_maps, eids_spell,
                eids_gtrans, eids_trans, eids_total, ner_type=ner_type, query_str=query_str, text=text, args=args)
    ...
    def get_lorelei_candidates(self, query_str, romanized_query_str=None, ner_type=None, args=None):
        ...
        return eids, eids_google, eids_hindi, eids_google_maps, eids_spell, eids_gtrans, eids_trans
