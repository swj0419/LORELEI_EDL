...
golddir=/pool0/webserver/incoming/experiment_tmp/EDL2019/data/gold/str0.2_filtered_wikiname/${lang}_edl_wiki.tab
overwrite=1
wikicg=1
google=1
google_top=5
...
TADIR=wikicg${wikicg}_google${google}_top${google_top}_map${google_map}_gtrans${g_trans}_tsl${tsl}_spell${spell}_inlink${inlink}_mtype${mtype}_clas${classifier}_bert${bert}_wc${wiki_contain}
...
echo ${NER_INDIR}
python link_entity_ranker.py --kbdir ${kbdir} --lang ${lang} --indir ${NER_INDIR} --outdir ${CANDGEN_OUTDIR} --pool ${pool} --year ${year} --overwrite ${overwrite} --wikicg ${wikicg} --google ${google} --google_top ${google_top} --google_map ${google_map} --g_trans ${g_trans} --tsl ${tsl} --spell ${spell} --inlink ${inlink} --mtype ${mtype} --classifier ${classifier} --bert ${bert} --wiki_contain ${wiki_contain}
cp link_entity_ranker.py ${OUTDIR}/link_entity_ranker.py
